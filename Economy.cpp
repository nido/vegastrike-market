...
//sell
void Economy::sellProduct(std::string what, int amount, double price, std::string who){
	this->markets[what]->addSellOrder(price,amount,who);
}

//buy
void Economy::buyProduct(std::string what, int amount, double price, std::string who){
...
bool Economy::executeTransactions(){
	bool executed= false;
	for( std::map <std::string,Market*>::const_iterator itr = this->markets.begin(); itr != this->markets.end(); ++itr){
...
void Economy::tick(){
	...
	//for each market
	for(std::map <std::string,Market*>::iterator itr =  markets.begin(); itr != markets.end(); ++itr){
cerr<<"Market: "<<(*itr).first<<endl;
	...
	//for each factory
	for(std::map <std::string,Factory*>::iterator itr =  factories.begin(); itr != factories.end(); ++itr){
cerr<<"Factory: "<<(*itr).first<<endl;
	...
	for(std::map <std::string,std::string>::iterator itr =  consumerMap.begin(); itr != consumerMap.end(); ++itr){
cerr<<"Consumer map: "<<(*itr).first<<" owned by "<<(*itr).second<<endl;
	...
	//for each consumer
	for(std::map <std::string,Consumer*>::iterator itr =  consumers.begin(); itr != consumers.end(); ++itr){
...
void Economy::placeSellOrders(std::string what, double loprice, int loamount,double hiprice,int hiamount, int steps){
...
//initialize a market by placing orders
void Economy::placeBuyOrders(std::string what, double loprice, int loamount,double hiprice,int hiamount, int steps){
	Market *pmarket = this->markets[what];
	for(int i=0; i<=steps; ++i){
...
void Economy::addConsumerMap(std::string factoryName,std::string consumerName){
