...
void Cargo::addCargo(const Cargo &newCargo) {
  ...
  for (newStock = newCargo.cargo.begin(); newStock != newCargo.cargo.end();
       ++newStock) {
...
bool Cargo::delCargo(const Cargo &newCargo) {
  ...
  for (Cargo::const_iterator newStock = newCargo.cargo.begin();
       newStock != newCargo.cargo.end(); ++newStock) {
...
unsigned int Cargo::getCount(const CargoType &type) const {
  ...
  std::map<CargoType, unsigned int>::const_iterator thing =
      this->cargo.find(type);
