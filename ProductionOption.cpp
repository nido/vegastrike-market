...
ProductionOption::ProductionOption(Products outputs, Products inputs,
                                   double sectorsize, double sectormin,
...
void ProductionOption::addElement(scew_element * root) const const
{
...
productionOptionFromElement(scew_element * root)
{
    ...
    if (NULL != poutputs && NULL != pinputs && 0.0 <= sectorsize
        && NULL != pcostofanother) {
        ppo =
            new ProductionOption(*poutputs, *pinputs, sectorsize,
                                 sectormin, *pcostofanother);
    }
    //clean up
    if (NULL != poutputs) {
        delete poutputs;
...
void Products_addElement(const Products & products, scew_element * root)
{
    ...
    for (Products::const_iterator pitr = products.begin();
         pitr != products.end(); ++pitr) {
