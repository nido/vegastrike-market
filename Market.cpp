...
//adds a sell order
void Market::addSellOrder(double price, int amount, string id, long precedence) {
	long time;
	...
	int buy_first_index= static_cast<int>(executedSellOrders.size());
	Elem *buy_order;
	...
	while( (buy_order = buy_orders.prev())!= NULL
		...
		executed_buy_order->m_payload.second = buy_order->m_payload.second;
		...
		executed_sell_order->m_payload.second = sell_order->m_payload.second;

		if( buy_order->m_payload.first < sell_order->m_payload.first){
			executed_sell_order->m_payload.first = buy_order->m_payload.first;
			executed_buy_order->m_payload.first = buy_order->m_payload.first;
		}else if( buy_order->m_payload.first > sell_order->m_payload.first){
			executed_sell_order->m_payload.first = sell_order->m_payload.first;
			executed_buy_order->m_payload.first = sell_order->m_payload.first;
		}else{
			executed_sell_order->m_payload.first = sell_order->m_payload.first;
			executed_buy_order->m_payload.first = buy_order->m_payload.first;
		}

        setLastDeal(buy_order->m_key, executed_buy_order->m_payload.first);

        //store difference of price(=first) in a structure for paying back sums paid in ahead
        Payback *payback = new Payback();
        payback->first= (buy_order->m_key - sell_order->m_key) * executed_buy_order->m_payload.first;
		...
		sell_order->m_payload.first -= executed_sell_order->m_payload.first;
	...
	for( int i=buy_first_index; i<static_cast<int>(executedBuyOrders.size()); ++i){
		Node *last;
...
//adds a buy order
void Market::addBuyOrder(double price, int amount, string id, long precedence) {
	long time;
	time = precedence>0?precedence:++timer;
	...
	int sell_first_index= static_cast<int>(executedBuyOrders.size());
	Elem *sell_order;
	...
	while( (sell_order = sell_orders.next())!= NULL
        ...
        setLastDeal(sell_order->m_key, executed_buy_order->m_payload.first);
	...
	for( int i=sell_first_index; i<static_cast<int>(executedSellOrders.size()); ++i){
		Node *last;
...
BTree_Iterator Market::getSellOrders() const{
	return BTree_Iterator(trackerSell.get_root(),true);
}

//gets the orders descending order
BTree_Iterator Market::getBuyOrders() const{
...
void Market::resetExecutedOrders() {
	for( int i=0; i<static_cast<int>(executedBuyOrders.size()); ++i){
		Elem *buy_order= executedBuyOrders[i];
        delete buy_order;
 	}
	executedBuyOrders.clear();

	for( int i=0; i<static_cast<int>(executedSellOrders.size()); ++i){
...
void Market::dump_summary(){
    ...
    cerr<<"["<<lowest_price<<","<<avg_value_of_trades()<<","<<highest_price<<"]"<<endl;
}
...
void Market::dumpExecutedOrders() {
	...
	for( int i=0; i<static_cast<int>(executedBuyOrders.size()); ++i){
		current= executedBuyOrders[i];
    		cerr << "price=" << current->m_key << " time=" << -current->m_secondkey << " amount=" << current->m_payload.first << " bidder=" << current->m_payload.second << " " << endl;
 	}
...
double Market::max_market_price_when_buying_n(int n,double &accumulated) {
	...
	while( (sell_order = sell_orders.next())!= NULL
		...
		if(n > sell_order->m_payload.first) {
			accumulated += sell_order->m_payload.first * sell_order->m_key;
		}else{
...
void Market::tick() {
	if(this->decayrate<1.0){
		...
		Elem *sell_order;
	        BTree_Iterator sell_orders = getSellOrders();
		while( (sell_order = sell_orders.next())!= NULL ){
			sell_order->m_payload.first = (int)floor(sell_order->m_payload.first*this->decayrate);
			if(sell_order->m_payload.first<=0){
		...
		//remove
		for(vector<Elem*>::const_iterator ritr = remove_orders.begin(); ritr != remove_orders.end(); ++ritr){
...
void Market::addElement(scew_element* root) const {
	...
	while( (current=iters.next())!= NULL ){
        ...
        scew_element_add_attr_pair(sub_element, "price", toCString(current->m_key) );
        scew_element_add_attr_pair(sub_element, "priority", toCString(current->m_secondkey) );
		sub_sub_element = scew_element_add(sub_element, "amount");
		scew_element_set_contents(sub_sub_element, toCString(current->m_payload.first));
		sub_sub_element = scew_element_add(sub_element, "bidder");
		scew_element_set_contents(sub_sub_element, current->m_payload.second.c_str());
	}
...
//    scew_attribute*attribute;
    ...
    if(NULL !=element){
        contents= scew_element_contents(element);
        if(NULL != contents){
            market->last_deal= doubleFromCString(contents,market->last_deal);
        }
    }
    ...
    if(NULL !=element){
        contents= scew_element_contents(element);
        if(NULL != contents){
            market->lowest_price= doubleFromCString(contents,market->lowest_price);
        }
    }
    ...
    if(NULL !=element){
        contents= scew_element_contents(element);
        if(NULL != contents){
            market->highest_price= doubleFromCString(contents,market->highest_price);
        }
    }
    ...
    if(NULL !=element){
        contents= scew_element_contents(element);
        if(NULL != contents){
            market->number_of_trades= doubleFromCString(contents,market->number_of_trades);
        }
    }
    ...
    if(NULL !=element){
        contents= scew_element_contents(element);
        if(NULL != contents){
            market->value_of_trades= doubleFromCString(contents,market->value_of_trades);
        }
    }
...
//pelem->dump();
                    ...
                    market->addSellOrder(pelem->m_key,pelem->m_payload.first,pelem->m_payload.second,pelem->m_secondkey);
                    delete pelem;
                }
            }
        }
        ...
        for(scew_element* sub_element=scew_element_next(element, NULL);NULL != sub_element;sub_element=scew_element_next(element, sub_element)){
            if(0==strcmp(scew_element_name(sub_element),"buy_order")){
                ...
                if(NULL != pelem){
                    ...
                    market->addBuyOrder(pelem->m_key,pelem->m_payload.first,pelem->m_payload.second,pelem->m_secondkey);
                    //TODO delete pelem;
                }
            }
        }
    ...
    if(NULL !=element){
        contents= scew_element_contents(element);
        if(NULL != contents){
            market->decayrate= doubleFromCString(contents,market->decayrate);
        }
    }
...
static Elem* elemFrom(scew_element * sub_element){
    ...
    for(scew_attribute* attribute=scew_attribute_next(sub_element, NULL);NULL != attribute;attribute=scew_attribute_next(sub_element, attribute)){
        ...
        }else if(0==strcmp(scew_attribute_name(attribute),"priority")){
    ...
    for(scew_element* sub_sub_element=scew_element_next(sub_element, NULL);NULL != sub_sub_element;sub_sub_element=scew_element_next(sub_element, sub_sub_element)){
        if(0==strcmp(scew_element_name(sub_sub_element),"amount")){
            amount= intFromCString(scew_element_contents(sub_sub_element));
        }else if(0==strcmp(scew_element_name(sub_sub_element),"bidder")){
